...
async function main(): Promise<number|undefined> {
    ...
    let client = new CriblControlPlane({ serverURL, debugLogger: console });
    ...
    for (let i=0; i<size; i++) {
      try {
        const res = await client.versions.commits.create({ gitCommitParams: { message: `fleet${i}`, group: `fleet${i}`}});
        console.log(res);
        await client.groups.deploy({
          product: ProductsCore.Edge, id: `fleet${i}`, deployRequest: { version: res.items?.[0]?.commit! }
        });
      } catch (error) {
        console.error(error);
      }
